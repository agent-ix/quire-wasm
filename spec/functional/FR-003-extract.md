...
| FR-003-AC-3 | `extractFromBlob(archetype, moduleBlob, doc)` returns the same shape as `extract(archetype, root, doc)` when `moduleBlob` is the in-memory equivalent of `root` ([FR-001](./FR-001-render.md) amendment, T-008). | Test |

## Dependencies

- **Upstream**: [StR-001](../stakeholder/StR-001-byte-identical-preview.md) (implements), `quire-rs/FR-011`, `quire-rs/FR-016` (wraps)
